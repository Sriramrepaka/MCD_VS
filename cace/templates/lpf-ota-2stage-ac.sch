...
ac dec 101 10 1MEG
meas ac acgain MAX vmag(v_out) FROM=100 TO=1k
let f3db = acgain/sqrt(2)
meas ac fc WHEN vmag(v_out)=f3db FALL=1

echo $&op1.dcgain $&fc > CACE\{simpath\}/CACE\{filename\}_CACE\{N\}.data
.endc
"}
...
C {devices/gnd.sym} 800 -420 0 0 {name=l2 lab=GND}
...
C {devices/gnd.sym} 1320 -460 0 0 {name=l4 lab=GND}
